...
internal void fill_sound_buffer(sound_output &sound_output, s32 samples_to_write,
                                game::sound_output_buffer &source_buffer)
{
    {
        BYTE *sound_buf_dat;
...
static void process_keyboard_message(input::key &new_state, const b32 is_down)
{
...
internal void process_Xinput_digital_button(DWORD Xinput_button_state_, input::button &old_state_,
                                            DWORD button_bit_, input::button &new_state)
{
...
internal void do_controller_input(game::game_input &old_input, game::game_input &new_input,
                                  HWND hwnd)
{
    // mouse cursor
    POINT mouse_point;
    ...
    for (DWORD controller_index = 0; controller_index < XUSER_MAX_COUNT; controller_index++) {
        input::controller &old_controller = old_input.controllers[controller_index];
        input::controller &new_controller = new_input.controllers[controller_index];
...
internal void process_pending_messages(win32_state &state, game::game_input &input)
{
    MSG msg;
...
s32 win32_main(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, s32 nShowCmd)
{
    ...
    game::game_input input[2]   = {};
    game::game_input *new_input = &input[0];
    game::game_input *old_input = &input[1];
    ...
    while (running) {
        ...
        game::sound_output_buffer sound_buffer = { .samples_per_second =
                                                       sound_output.samples_per_sec,
                                                   .sample_count = samples_to_write,
                                                   .samples      = samples };

        game::offscreen_buffer buffer = { .memory          = g_back_buffer.memory,
                                          .width           = g_back_buffer.width,
                                          .height          = g_back_buffer.height,
                                          .pitch           = g_back_buffer.pitch,
                                          .bytes_per_pixel = 4 };

        game::update(&thread, &memory, *input, delta_time);
...
// NOTE: this is debug code
        ...
        game::game_input *temp_input = new_input;
        new_input                    = old_input;
